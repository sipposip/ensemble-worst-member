...
for date in pd.date_range('201906011200','201908251200', freq='5d'):
    ...
    plt.savefig(f'plots/angle_vs_amplitude_mvn_uncertainty_{datestr}_{leadtime}h.svg')

    bootstrap_res.to_pickle(f'data/angle_vs_amplitude_mvn_uncertainty_{datestr}_{leadtime}h.pkl')

    print(bootstrap_res.groupby('method').std()[['angle','a']])
    print(bootstrap_res.groupby('method').std()[['angle','a']],
          file=open(f'data/angle_vs_amplitude_mvn_uncertainty_{datestr}_{leadtime}h_stdev.txt','w'))
