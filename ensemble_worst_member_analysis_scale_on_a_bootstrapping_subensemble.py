...
for date in pd.date_range('201906011200','201908251200', freq='5d'):
    ...
    for i_bootstrap in trange(n_bootstrap):
        ...
        mem_idcs = np.random.randint(0,n_ens, size=sub_ens_size)
    ...
    plt.savefig(f'plots/angle_vs_amplitude_bootstrap_subens_{sub_ens_size}_{datestr}_{leadtime}h.svg')

    bootstrap_res.to_pickle(f'data/angle_vs_amplitude_bootstrap_subens_{sub_ens_size}_{datestr}_{leadtime}h.pkl')

    print(bootstrap_res.groupby('method').std()[['angle','a']])
    print(bootstrap_res.groupby('method').std()[['angle','a']],
          file=open(f'data/angle_vs_amplitude_bootstrap_subens_{sub_ens_size}_{datestr}_{leadtime}h_stdev.txt','w'))