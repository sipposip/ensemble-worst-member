...
def corr_over_time(x,y):
    ...
    # on a lower level (only with mean and std functions)
    mx = x.mean('time')
    my = y.mean('time')
    xm, ym = x-mx, y-my
...
def seldate(data,date):
    '''select a single timestep. date can be string or anything that pandas recognizes
    (e.g. Timestamps)'''
...
def area_func(data,func='mean', **kwargs):
    '''
        apply a function "func" over the area of the data
        func: string, name of an xarray.DataArray function
