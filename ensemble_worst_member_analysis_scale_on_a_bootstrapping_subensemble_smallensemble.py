...
for date in pd.date_range('201906011200','201908251200', freq='5d'):
    ...
    plt.savefig(f'plots/angle_vs_amplitude_bootstrap_subens_{sub_ens_size}_{datestr}_{leadtime}h_smallense{n_ens_sub}.svg')

    bootstrap_res.to_pickle(f'data/angle_vs_amplitude_bootstrap_subens_{sub_ens_size}_{datestr}_{leadtime}h_smallense{n_ens_sub}.pkl')

    print(bootstrap_res.groupby('method').std()[['angle','a']])
    print(bootstrap_res.groupby('method').std()[['angle','a']],
          file=open(f'data/angle_vs_amplitude_bootstrap_subens_{sub_ens_size}_{datestr}_{leadtime}h_stdev_smallense{n_ens_sub}.txt','w'))